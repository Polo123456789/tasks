...
- En Inicio, Vencimiento y el filtro de fechas, `Ctrl+O` abre un calendario sin descartar la entrada manual. Las flechas avanzan por días, `PgUp`/`PgDn` por semanas, `[`/`]` por meses y `Home` vuelve a hoy; `Enter` elige, `x` limpia y `Esc` regresa sin cambiar el borrador.
